...
from .generation_utils import build_chat_input, build_chat_input_batch, TextIterStreamer
...
class BaichuanForCausalLM(BaichuanPreTrainedModel):
    ...
    @torch.no_grad()
    def chat_batch(self, tokenizer, messages_batch: List[List[dict]], generation_config: Optional[GenerationConfig]=None):
        generation_config = generation_config or self.generation_config
        input_ids_batch, batch_max_len = build_chat_input_batch(self, tokenizer, messages_batch, generation_config.max_new_tokens)
        outputs_batch = self.generate(input_ids_batch, generation_config=generation_config)
        response_batch = []
        for outputs in outputs_batch:
            response = tokenizer.decode(outputs[batch_max_len:], skip_special_tokens=True)
